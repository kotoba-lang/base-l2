(ns kotoba.lang.base-l2.abi-test
  "JVM-only (mirrors `kotoba.lang.base-l2.abi`, which it tests): uses
  `clojure.java.io`/`io/resource` to load fixtures and `eth-crypto.core`
  directly, both JVM-only already, on top of `abi.clj` itself being
  JVM-only (see its docstring).
